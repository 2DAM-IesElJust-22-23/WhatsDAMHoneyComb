...
public class ServerMsg {
    ...
    public static void listen(int srvPort) throws IOException {
        ...
        try{
            ...
            System.out.println("> El port " + srvPort + "està ocupat o es inaccessible.");
            return;
        }
        
        // 2. Iniciem un bucle infinit a l'espera de rebre connexions    
        
        System.out.println("Aranque server...");
        while(true){
            
            Socket socket = listener.accept();
            System.out.println("> S'ha establit la connexió.");
            
            MsgHandler handler = new MsgHandler(socket,Connexions);
            Thread fil = new Thread(handler);
            fil.start();            
        }
